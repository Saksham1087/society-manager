(function () {
  ...
  document.getElementById('btn-backup-gdrive').addEventListener('click', function () {
    doCloudBackup('https://drive.google.com/drive/my-drive');
  });
  document.getElementById('btn-backup-dropbox').addEventListener('click', function () {
    doCloudBackup('https://www.dropbox.com/home');
  });
  document.getElementById('btn-backup-onedrive').addEventListener('click', function () {
    doCloudBackup('https://onedrive.live.com/');
  });

  // ========== UNIFIED CLOUD RESTORE ==========
  var cloudRestoreInput = document.createElement('input');
  cloudRestoreInput.type = 'file';
  cloudRestoreInput.accept = '.json';
  cloudRestoreInput.style.display = 'none';
  document.body.appendChild(cloudRestoreInput);

  cloudRestoreInput.addEventListener('change', function () {
    var file = this.files[0];
    if (!file) return;
    var reader = new FileReader();
    reader.onload = function (e) {
      if (importDataPayload(e.target.result)) {
        location.reload();
      }
    };
    reader.readAsText(file);
  ...
  function doCloudRestore() {
    alert('To restore: 1. Download your backup JSON file from your cloud account to your device. 2. Click \'Choose File\' below to process the recovery update.');
    cloudRestoreInput.click();
  }

  document.getElementById('btn-restore-gdrive').addEventListener('click', doCloudRestore);
  document.getElementById('btn-restore-dropbox').addEventListener('click', doCloudRestore);
  document.getElementById('btn-restore-onedrive').addEventListener('click', doCloudRestore);
