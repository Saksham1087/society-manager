(function () {
  ...
  function buildExpenseVoucherMsg(exp) {
    return 'Dear ' + exp.vendor + ', an expense payment of \u20B9' + Number(exp.amount).toFixed(2) + ' has been processed for ' + exp.category + ' via ' + exp.mode + ' on ' + exp.date + '.' + (exp.description ? ' Notes: ' + exp.description + '.' : '') + ' - Generated by Society Management System';
  }

  document.getElementById('btn-expense-whatsapp').addEventListener('click', function () {
    if (!_currentVoucherExpense) return;
    var msg = buildExpenseVoucherMsg(_currentVoucherExpense);
    window.open('https://api.whatsapp.com/send?text=' + encodeURIComponent(msg), '_blank');
  });

  document.getElementById('btn-expense-sms').addEventListener('click', function () {
    if (!_currentVoucherExpense) return;
    var msg = buildExpenseVoucherMsg(_currentVoucherExpense);
